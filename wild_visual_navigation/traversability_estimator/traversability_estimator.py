...
class TraversabilityEstimator:
    def __init__(
        ...
        extraction_store_foler=None,
        **kwargs,
    ):
        self._device = device
        self._mode = mode
        self._extraction_store_foler = extraction_store_foler
    ...
    def add_mission_node(self, node: MissionNode, verbose: bool = False):
        ...
        if self._mission_graph.add_node(node) and node.use_for_training:
            ...
            if self._mode == WVNMode.EXTRACT_LABELS:
                p = os.path.join(self._extraction_store_foler, "image", str(node.timestamp).replace(".", "_") + ".pt")
