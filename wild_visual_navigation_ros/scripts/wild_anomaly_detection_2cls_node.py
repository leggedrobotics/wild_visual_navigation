#!/usr/bin/env python3
"""
Wild anomaly detection inference node, method 2.
...
class AnomalyInference(object):
    def __init__(self):
        ...
        self.loss_scaling = rospy.get_param("~loss_scaling", default=0.25)
        ...
        self.info_input_topic = rospy.get_param("~info_input_topic", default="/wide_angle_camera_front/camera_info")
        ...
        self.weights_path = os.path.join(WAD_ROOT_DIR, "assets", "weights", "2cls", rospy.get_param("~weights", default=""))
        self._cv_bridge = CvBridge()

        self.new_info_msg = None

        # Load model
        self.model = load_model_2cls(model_path=self.weights_path, device=DEVICE, feat_size=384, batch_size=100)
    ...
    def inference(self, feat, seg):
        with torch.no_grad():
            ...
            # Convert to binary
            losses = torch.sigmoid(self.loss_scaling * y_pred).squeeze()
